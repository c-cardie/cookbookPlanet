...
function displayRecipe(index) {
  ...
  if (recipe) {
    // Fetch recipe stories
    fetch("../static/recipe/All_recipe_stories.json")
      .then((response) => response.json())
      .then((storiesData) => {
        const story = storiesData.find((s) => s.id === recipe.id)?.story || "No story available for this recipe.";
      ...
      .catch((error) => console.error("Error fetching recipe stories:", error));
  }
}
